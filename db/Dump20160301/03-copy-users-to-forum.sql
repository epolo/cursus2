...
insert into forum.smf_members (
	...
	'', '', '', '', ''
 from cursus2.users 
 where email not in (select email_address from forum.smf_members);

update cursus2.users u set u.php_id=(select id_member from forum.smf_members where email_address=u.email);
